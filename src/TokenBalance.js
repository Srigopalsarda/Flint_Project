...
const TokenBalance = () => {
    ...
    // fetch balance of native token of the chain
    const fetchBalance = () => {
    ...
    // fetch % change in native token of the chain
    const fetchBalance2 = () => {
        let headers = new Headers();
        headers.set('Authorization', "Bearer cqt_rQY3kTGRY7pwfPRvVQ7XgwKrYpQg");
        fetch(`https://api.covalenthq.com/v1/${chain}-mainnet/address/${address}/portfolio_v2/?`, { method: 'GET', headers: headers })
            ...
            .then((data) => {
                console.log(data);
                let changeinprice = data.data.items[0].holdings[0].close.quote - data.data.items[0].holdings[1].close.quote; //change in price of native token in last 24 hours according to api
                let percentinc_or_dec = ((changeinprice / data.data.items[0].holdings[1].close.quote) * 100); // % change in price of native token in last 24 hours according to api (checking increment or decrement)
